...
def add_category(app_log):
    ...
    if category is None:
        ...
        if app_log.get('status').find('500 Server closed connection') > 0:
            category = '500'
        elif app_log.get('status').find('exit code is 123') >= 0 or app_log.get('status').find('No space left on device') >= 0:
            category = 'NoSpace'
        ...
        # User Errors
            ...
            category = 'User Error'
        elif app_log.get('status').find('Feature ID') >= 0 and app_log.get('status').find('does not exist in the supplied genome') >= 0:
            category = 'User Error'
#        Assembly
...
+            category = 'User Error'
            ...
            category = 'User Error'
        elif app_log.get('status').find('Parent ID') >= 0 and app_log.get('status').find('was not found in feature ID list') >= 0:
            category = 'User Error'
